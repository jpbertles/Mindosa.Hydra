...
namespace Mindosa.Hydra.Internal
{
    class PocoData
    {
        ...
        public Delegate GetFactory(string typeName, string keyName, int firstColumn, int countColumns, IDataReader r)
        {
            // Check cache
            var key = Tuple.Create<string, string, int, int>(typeName, keyName, firstColumn, countColumns);
